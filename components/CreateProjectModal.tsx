...
export default function CreateProjectModal({ isOpen, onClose }: CreateProjectModalProps) {
  ...
  useEffect(() => {
    if (!isOpen) {
      // Delay reset slightly to let the exit animation finish smoothly
  ...
  const handleContinue = async () => {
    ...
    if (step < 5) {
      ...
      // Step 5 → Create Project → show loader → navigate
      setIsCreating(true);
      setTimeout(() => {
        router.push("/canvas");
      }, 2000);
          ...
          >
            ...
            {step === 2 && (
              <div className="flex flex-col gap-4 h-full">
                ...
                <button className="text-[14px] font-medium text-[#004643] hover:underline self-start">
              ...
              <div className="flex gap-[24px] h-full">
                    ...
                    {rooms.length > 0 && (
                      <div className="flex flex-col gap-[7px] w-[43%]">
                        {rooms.slice(0, 1).map((room) => (
                           ...
                           >
                             <p className="font-semibold text-[#004643] text-[13px] text-center px-1">
                               {room.name}
                             </p>
                           </div>
                        ))}
                ...
                <div className="bg-white border border-[#f1f4f4] h-full min-h-[350px] overflow-hidden relative rounded-[12px] w-[240px] shrink-0 flex flex-col">
                  <div className="bg-white border-b border-[#f1f4f4] flex items-center justify-between h-[44px] shrink-0 px-[16px]">
                    <p className="font-semibold text-[#101212] text-[13px]">
                      {rooms.length} rooms
                    </p>
                    <button onClick={handleAddRoom} className="font-semibold text-[#004643] text-[13px] hover:underline">
                      + Add
                    </button>
                  </div>
                  <div className="overflow-y-auto flex-1 hide-scrollbar">
                    {rooms.map((room, index) => (
                      ...
                      >
                        <div 
                          className="border border-[rgba(0,0,0,0.08)] rounded-[3px] w-[12px] h-[12px] shrink-0" 
              ...
              <div className="flex flex-col gap-4">
                <div className="flex items-center justify-between bg-[#fafafa] px-4 py-2 rounded-xl border border-gray-200">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button className="bg-gray-100 hover:bg-gray-200 text-[#0a0a0a] text-[13px] font-medium px-4 h-8 rounded-[8px] transition-colors">
                      Apply
                    </button>
                  </div>
                </div>
