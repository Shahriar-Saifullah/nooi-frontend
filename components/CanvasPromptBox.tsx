...
export default function CanvasPromptBox({
  ...
  return (
    <div ref={containerRef} className="px-3 pb-4" style={{ position: "relative", zIndex: 50 }}>
      <div
      ...
      >
        ...
        {/* Bottom toolbar */}
        <div className="flex items-center justify-between mt-1" style={{ position: "relative", zIndex: 9999, overflow: "visible" }}>
          ...
          <div className="flex items-center gap-[6px]">
            ...
            <label className="w-[28px] h-[28px] rounded-[8px] flex items-center justify-center hover:bg-[#f5f5f5] transition-colors cursor-pointer text-[#6b7280]">
              ...
              <input type="file" accept="image/*,.pdf" className="hidden" onChange={(e) => { if (e.target.files && e.target.files.length > 0) setAttachedFile(e.target.files[0].name); }} />
            </label>
            ...
            <div style={{ position: "relative" }}>
              <button
                ...
                onMouseDown={(e) => {
                  ...
                  if (!modelMenuOpen && modelButtonRef.current) {
                    ...
                    setModelMenuOpensUp(spaceBelow < 200);
                  }
              ...
              {modelMenuOpen && (
                <div
                  style={{
                    position: "absolute",
                    ...(modelMenuOpensUp ? { bottom: "100%", marginBottom: 4 } : { top: "100%", marginTop: 4 }),
                    left: 0,
                    width: 160,
                    zIndex: 9999,
            ...
            <div style={{ position: "relative" }}>
              ...
              {mentionMenuOpen && (
                <div style={{ position: "absolute", top: "100%", left: 0, marginTop: 4, width: 160, zIndex: 9999 }} className="bg-white border border-[#e5e5e5] rounded-[8px] shadow-[0px_8px_24px_rgba(0,0,0,0.15)] overflow-hidden">
                  {["@Project_Alpha", "@JohnDoe", "@DesignTeam"].map(mention => (
                    <button key={mention} type="button" onMouseDown={(e) => { e.preventDefault(); setDesignPrompt(p => p + (p.length > 0 && !p.endsWith(" ") ? " " : "") + mention + " "); setMentionMenuOpen(false); }} className="w-full text-left px-3 py-2 text-[11px] font-medium text-[#374151] hover:bg-[#f5f5f5] transition-colors border-b border-[#f5f5f5] last:border-0 cursor-pointer">
                      {mention}
                    </button>
                  ))}
                </div>
          ...
          <div className="flex items-center gap-[6px]">
            <button
              ...
              className={`w-[28px] h-[28px] rounded-[8px] flex items-center justify-center transition-all duration-300 cursor-pointer ${isRecording ? "opacity-100 scale-110 bg-red-50 text-red-500" : "opacity-60 hover:opacity-100 hover:bg-[#f5f5f5] text-[#374151]"}`}
            >
              <svg className={isRecording ? "animate-pulse" : ""} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <button
              ...
              className={`w-[28px] h-[28px] rounded-full flex items-center justify-center text-white transition-all duration-300 shadow-sm ${isPromptActive && !isGenerating ? "bg-[#004643] hover:bg-[#003330] cursor-pointer" : "bg-[#e5e5e5] text-[#a3a3a3] cursor-not-allowed shadow-none"}`}
            >
              {isGenerating ? (
                <Loader2 size={13} className="animate-spin" />
              ) : (
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className={isPromptActive ? "" : "opacity-60"}>
