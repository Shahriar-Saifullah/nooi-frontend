...
const aiTools = [
  ...
  },
  {
    ...
    desc: "Write what you imagine, and come into beautiful interior visuals.",
    badge: "Make modern bathroom",
    splitPos: "27.6%",
    imageContent: (
      <div className="relative w-full h-full overflow-hidden">
...
export default function DashboardPage() {
  ...
  const FilterTabBar = ({
    ...
    <div className="bg-[#f5f5f5] h-10 flex items-center p-1 rounded-full overflow-x-auto hide-scrollbar shrink-0">
      {filterTabs.map((tab) => (
        <button
          key={tab}
  ...
  const CreationCard = ({ item }: { item: typeof recentCreations[0] }) => (
    <div className="border border-[#f0f0f0] rounded-[20px] p-2 flex flex-col group cursor-pointer hover:shadow-md transition-all min-w-0">
      ...
      <div className="px-1 sm:px-2 pb-1 flex flex-col self-stretch min-w-0">
  ...
  return (
    <div className="min-h-screen w-full overflow-x-hidden bg-[#fafafa] font-inter text-[#0a0a0a] tracking-[-0.36px] leading-[1.5]">
      {/* ── Navbar ── */}
      <header className="bg-white px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between shadow-sm sticky top-0 z-50 min-w-0">
        <div className="flex items-center gap-3 min-w-0">
          <div className="flex items-center shrink-0">
            <Image width={100} height={100} src="/Logo/Logo.svg" alt="Nooi" className="h-6 w-auto" />
          </div>

          <div className="hidden md:flex bg-[#f5f5f5] h-10 items-center p-1 rounded-full shrink-0">
            {(["home", "history", "collection"] as const).map((tab) => {
              const icons = { home: <Home className="w-4 h-4" />, history: <Clock className="w-4 h-4" />, collection: <FolderOpen className="w-4 h-4" /> };
              const labels = { home: "Home", history: "History", collection: "Collection" };
              return (
                <button
                  key={tab}
                  onClick={() => setActiveNavTab(tab)}
                  className={`flex items-center gap-2 h-full px-4 rounded-full transition-all text-xs font-medium ${
                    activeNavTab === tab
        ...
        <div className="flex items-center gap-2 sm:gap-4 shrink-0">
          <button className="hidden lg:flex items-center gap-2 px-4 h-10 rounded-full hover:bg-gray-50 transition-colors">
          ...
          <div className="relative" ref={dropdownRef}>
            <div
              className="bg-[#f5f5f5] flex items-center gap-2 py-1 pl-1 pr-3 rounded-full shadow-sm cursor-pointer hover:bg-[#efefef] transition-colors select-none"
              onClick={() => setDropdownOpen(prev => !prev)}
            >
              {user?.avatar_url ? (
                <Image width={100} height={100} src={user.avatar_url} alt={user.full_name} className="w-8 h-8 rounded-full object-cover shrink-0" />
              ) : (
                <Image width={100} height={100} src="/assets/imgAvatar.png" alt={user?.full_name || "User"} className="w-8 h-8 rounded-full object-cover shrink-0" />
              )}
              <span className="text-xs font-medium hidden sm:block max-w-[120px] truncate">{user?.full_name || "User"}</span>
              <ChevronRight className={`w-4 h-4 text-gray-500 shrink-0 transition-transform duration-200 ${dropdownOpen ? "rotate-90" : ""}`} />
            </div>
            ...
            <AnimatePresence>
              {dropdownOpen && (
                ...
                >
                  ...
                  <div className="block md:hidden border-b border-[#f5f5f5]">
                    {(["home", "history", "collection"] as const).map((tab) => {
                      const icons = { home: <Home className="w-4 h-4 text-[#737373]" />, history: <Clock className="w-4 h-4 text-[#737373]" />, collection: <FolderOpen className="w-4 h-4 text-[#737373]" /> };
                      const labels = { home: "Home", history: "History", collection: "Collection" };
                      return (
                        <button
                          key={tab}
                          className={`w-full flex items-center gap-3 px-4 py-2.5 text-[13px] transition-colors ${
                            activeNavTab === tab ? "bg-[#f5f5f5] text-[#0a0a0a] font-semibold" : "text-[#737373] hover:bg-[#f5f5f5]"
                          }`}
                          onClick={() => { setActiveNavTab(tab); setDropdownOpen(false); }}
                        >
                          {icons[tab]}
                          {labels[tab]}
                        </button>
                  ...
                  <div className="py-1.5">
                    <button className="w-full flex items-center gap-3 px-4 py-2.5 text-[13px] text-[#0a0a0a] hover:bg-[#f5f5f5] transition-colors" onClick={() => setDropdownOpen(false)}>
                      <User className="w-4 h-4 text-[#737373]" /> Profile
                    </button>
                    <button className="w-full flex items-center gap-3 px-4 py-2.5 text-[13px] text-[#0a0a0a] hover:bg-[#f5f5f5] transition-colors" onClick={() => setDropdownOpen(false)}>
                      <Settings className="w-4 h-4 text-[#737373]" /> Settings
                    </button>
                  </div>
                  <div className="border-t border-[#f5f5f5] py-1.5">
                    <button className="w-full flex items-center gap-3 px-4 py-2.5 text-[13px] text-red-500 hover:bg-red-50 transition-colors" onClick={handleLogout}>
      ...
      {/* ── Main Layout ── */}
      <main className="w-full max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col xl:flex-row gap-6">

        {/* Left Column */}
        <motion.div
          className="flex-1 min-w-0 bg-white rounded-3xl shadow-[0_0_32px_rgba(149,157,165,0.04)] p-4 sm:p-6 flex flex-col gap-4 sm:gap-6"
        ...
        >
          ...
          {activeNavTab === "home" && (
            <>
              ...
              {/* Quick Actions */}
              <motion.div variants={itemVariants} className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
                {[
                  ...
                  <div key={i} onClick={() => setCreateModalOpen(true)} className="relative flex items-center gap-3 p-3 pr-8 border border-[#f0f0f0] rounded-2xl hover:shadow-sm transition-shadow cursor-pointer min-w-0">
                    ...
                    <ArrowUpRight className="w-3.5 h-3.5 text-gray-400 absolute top-3 right-3 shrink-0" />
                  </div>
                ))}
              </motion.div>
              ...
              <AnimatePresence>
                {showFreePlanBanner && (
                  <motion.div
                    ...
                    className="bg-[#fcfdf8] border border-[#f0f4e3] rounded-2xl p-3 sm:p-4 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 min-w-0"
                  >
              ...
              <motion.div variants={itemVariants} className="flex flex-col gap-3">
                ...
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <FilterTabBar activeTab={creationsFilter} onTabChange={setCreationsFilter} />
                  <button
                    onClick={() => setActiveNavTab("history")}
                    className="text-[#004643] text-[12px] font-medium flex items-center gap-1 bg-[#f5f5f5] h-8 px-3 rounded-full hover:bg-gray-200 shrink-0 transition-colors"
                  >
                ...
                <AnimatePresence mode="wait">
                  ...
                  >
                    {filteredCreations.length > 0 ? filteredCreations.map((item, i) => (
                      <CreationCard key={`${item.title}-${i}`} item={item} />
                    )) : (
                      <div className="col-span-2 lg:col-span-4 flex flex-col items-center justify-center py-10 text-center text-[#737373] border border-dashed border-gray-200 rounded-2xl">
              ...
              <motion.div variants={itemVariants} className="flex flex-col gap-3">
                ...
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <FilterTabBar activeTab={aiToolsFilter} onTabChange={setAiToolsFilter} />
                  <button className="text-[#004643] text-[12px] font-medium flex items-center gap-1 bg-[#f5f5f5] h-8 px-3 rounded-full hover:bg-gray-200 shrink-0 transition-colors">
                ...
                <AnimatePresence mode="wait">
                  <motion.div
                    ...
                    className="grid grid-cols-2 lg:grid-cols-4 gap-3"
                  >
                    {filteredAiTools.length > 0 ? filteredAiTools.map((item, i) => (
                      <div key={`${item.title}-${i}`} className="border border-[#f0f0f0] rounded-[20px] p-2 flex flex-col group cursor-pointer hover:shadow-md transition-all min-w-0">
                        <div className="rounded-xl h-[100px] sm:h-[120px] mb-2 sm:mb-3 relative overflow-hidden bg-gray-100">
                          ...
                          {item.badge && (
                            <div className="absolute bottom-2 left-1/2 -translate-x-1/2 bg-[rgba(255,255,255,0.8)] backdrop-blur-md px-2 py-1.5 rounded-full flex items-center gap-1.5 shadow-sm border border-[#c7de7d] z-10">
                        ...
                        <div className="px-1 flex-1 flex flex-col min-w-0">
                          <h4 className="text-[13px] sm:text-[15px] font-medium leading-[1.5] tracking-[-0.48px] text-[#0a0a0a] mb-1 truncate">{item.title}</h4>
                          <p className="text-[12px] sm:text-[13px] font-normal leading-[1.5] tracking-[-0.42px] text-[#404040]">{item.desc}</p>
                        </div>
                      </div>
                    )) : (
                      <div className="col-span-2 lg:col-span-4 flex flex-col items-center justify-center py-10 text-center text-[#737373] border border-dashed border-gray-200 rounded-2xl">
        ...
        {/* Right Column (Sidebar) — hidden below xl, shown stacked on smaller screens */}
        <motion.aside
          className="w-full xl:w-[340px] 2xl:w-[380px] shrink-0 bg-white rounded-3xl shadow-[0_0_32px_rgba(149,157,165,0.04)] flex flex-col min-w-0 overflow-hidden"
          variants={containerVariants}
          initial="hidden"
          animate="show"
        >
          <div className="p-5 flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <h2 className="text-[18px] font-[600] tracking-[-0.6px] text-[#0a0a0a]">Tutorials For You</h2>
              <button className="bg-[#f5f5f5] h-9 px-4 rounded-full flex items-center gap-2 hover:bg-gray-200 transition-colors shrink-0">
                <Image width={100} height={100} src="/assets/imgGroup1.svg" className="w-4 h-4" alt="help" />
                <span className="text-[13px] font-medium text-[#0a0a0a] tracking-[-0.42px]">Help</span>
              </button>
            </div>
          ...
          <div className="flex-1 overflow-y-auto px-5 pb-5 flex flex-col gap-8 hide-scrollbar">
            {/* Tutorial 1 */}
            <motion.div variants={itemVariants} className="flex flex-col gap-3 group cursor-pointer">
              <div className="aspect-[1.5] relative bg-[#f5f5f5] rounded-[28px] overflow-hidden">
                <div className="absolute left-[7.5%] bottom-[11.3%] w-[60.2%] h-[58.8%] rounded-[14px] overflow-hidden border border-white shadow-sm">
                  <Image fill src="/assets/imgImage16.png" alt="Before" className="object-cover" style={{ objectPosition: "-8.595px 0px" }} />
                </div>
                <div className="absolute right-[7.5%] top-[4.5%] w-[48.2%] h-[54.3%] rounded-[14px] overflow-hidden border border-white shadow-[0_2px_8px_rgba(99,99,99,0.12)] z-10">
                ...
                <div className="absolute bottom-[3%] left-1/2 -translate-x-1/2 bg-white px-3 py-2 rounded-full flex items-center justify-center gap-2 border-[1.5px] border-[#d2e88a] shadow-[0_4px_12px_rgba(0,0,0,0.08)] z-30 whitespace-nowrap max-w-[90%]">
                  <Image width={100} height={100} src="/assets/imgRating2.svg" className="w-4 h-4 shrink-0" alt="sparkle" />
                  <span className="text-[10px] font-[500] tracking-[-0.42px] text-[#0a0a0a] truncate">Add picture in the wall</span>
                </div>
              </div>
              <p className="text-[15px] font-[600] text-[#0a0a0a] tracking-[-0.48px] leading-[1.4]">Transform your empty wall into a stylish focal point in just one click.</p>
            </motion.div>

            {/* Tutorial 2 */}
            <motion.div variants={itemVariants} className="flex flex-col gap-3 group cursor-pointer">
              <div className="aspect-[16/9] relative bg-[#f5f5f5] rounded-[22px] flex items-center justify-center p-3 overflow-hidden">
                <div className="relative w-full h-full flex items-start gap-2">
                  <div className="flex-1 h-[80%] rounded-[10px] overflow-hidden shadow-md border-2 border-white relative">
                    <Image fill src="/assets/imgImage9.png" alt="Sketch" className="absolute inset-0 object-cover" />
                    <div className="absolute bottom-2 left-1/2 -translate-x-1/2 bg-white/95 px-2 py-1 rounded-lg text-[10px] font-medium shadow-sm whitespace-nowrap z-10">Sketch</div>
                  </div>
                  <div className="flex-[1.3] h-full rounded-[10px] overflow-hidden shadow-md border-2 border-white relative">
                    <Image fill src="/assets/imgImage8.png" alt="Render" className="absolute inset-0 object-cover" />
                    <div className="absolute bottom-2 left-1/2 -translate-x-1/2 bg-white/95 px-2 py-1 rounded-lg text-[10px] font-medium shadow-sm whitespace-nowrap z-10">Render result</div>
                  </div>
                </div>
              </div>
            ...
            <motion.div variants={itemVariants} className="flex flex-col gap-3 group cursor-pointer">
              <div className="aspect-[1.5] relative bg-[#f5f5f5] rounded-[28px] overflow-hidden">
                <div className="absolute left-[6%] top-[18.1%] w-[42.8%] h-[48%] rounded-[14px] overflow-hidden border border-white shadow-[0_2px_8px_rgba(99,99,99,0.12)] z-20">
                ...
                <div className="absolute left-[52.7%] top-[29.4%] w-[42.8%] h-[47%] rounded-[14px] overflow-hidden border border-white shadow-[0_2px_8px_rgba(99,99,99,0.12)] z-0">
                ...
                <div className="absolute left-[28.6%] top-[65.6%] w-[19.6%] h-[20.8%] flex items-center justify-center z-50">
                  ...
                  <span className="relative text-[10px] font-bold text-[#c7de7d] tracking-[-0.3px]">Brush..</span>
                </div>
              </div>
              <p className="text-[15px] font-[600] text-[#0a0a0a] tracking-[-0.48px] leading-[1.4]">Easily switch colors and tones to explore new design moods.</p>
            </motion.div>
          </div>
        </motion.aside>
      </main>
